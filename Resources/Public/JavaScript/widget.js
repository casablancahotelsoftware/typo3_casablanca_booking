...
(function () {
    ...
    function createRoomBlock(index) {
        ...
        adultsLabel.innerHTML = '<span class="cb-field__label">' + cbText('adults', 'Adults') + '</span>'
            + '<input type="number" name="rooms[' + index + '][adults]" min="1" max="10" value="2" data-cb-field="adults" required />';
        ...
        childrenLabel.innerHTML = '<span class="cb-field__label">' + cbText('children', 'Children') + '</span>'
            + '<input type="number" name="rooms[' + index + '][children]" min="0" max="10" value="0" data-cb-field="children-count" required />';
    ...
    function syncRoomBlocks(form, options) {
        ...
        while (blocks.length < count) {
            var block = createRoomBlock(blocks.length);
    ...
    function getCalendarDefaultOccupancy(form) {
        ...
        return {
            adults: clamp(form.getAttribute('data-cb-default-adults') || '2', 1, 99),
            children: clamp(form.getAttribute('data-cb-default-children') || '0', 0, 99)
        };
    }
    ...
    function createCompactCalendarRoomBlock(index, defaults) {
        defaults = defaults || { adults: 1, children: 0 };
        ...
        block.innerHTML = headerHtml
            ...
            + '<input type="hidden" name="rooms[' + index + '][childrenAges]" value="" data-cb-children-ages-hidden />'
            ...
            + '<div class="cb-field cb-field--ages" data-cb-ages-container hidden="hidden">'
    ...
    function initCalendarSelection(form) {
        ...
        state.nextPick = 'from';
        state.fromMinStay = 1;
        state.visibleCount = clamp(parseInt(form.getAttribute('data-cb-initial-months') || '1', 10), 1, 2);
